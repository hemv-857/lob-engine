...
fn main() {
    ...
    let tape: Vec<TapeTick> = (0..n)
        .map(|i| {
            ...
            TapeTick { qty: (50.0 + 450.0 * (tod * 2.0).abs()) as u64, price }
        })
        .collect();

    println!("parent: buy 50_000 over {} ticks\n", n);
    println!("{:<6} {:>12} {:>14}", "algo", "filled", "slippage_bps");
    for (algo, pov) in [(Algo::Twap, 0.2), (Algo::Vwap, 0.2), (Algo::Pov, 0.1), (Algo::Pov, 0.3)] {
        let cfg = AgentConfig {
            algo,
            side: ExecSide::Buy,
            parent_qty: 50_000,
        ...
        let label = if algo == Algo::Pov { format!("{:?}-{}", algo, pov) } else { format!("{algo:?}") };
